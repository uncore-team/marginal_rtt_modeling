...
% S -> entire scenario
% TIT -> title
% REGS -> regimes detected (see RegimesDetection); empty if none.
...
% HFIGESC <- figure handler for the one that shows the scenario.
    ...
    figure;
    plot(S/1000,'.-','LineWidth',2); % assume millisecs in the RTTs
    grid;
    xlabel('RTT index');
    ylabel('RTT (s)');
    ...
    hold on;
    grid;
    plotregs([1:length(S)],S,regs,1); % 1 for minimuminfo (for the paper); alternative: ts in X, but we can miss visual information
...
function plotregs(ts,S,regs,minimuminfo)
    ...
    if nr == 0 % the entire data sequence is a non-regime
        drawOneRegime(0,ts,S,1,N,[],minimuminfo);
    else
        if regs(1,1) > 1 % heading non-regime
            drawOneRegime(0,ts,S,1,regs(1,1)-1,[],minimuminfo);
        end
        for f = 1:nr
            indini = regs(f,1);
            indfini = regs(f,2);
            if (f > 1) && (indini > regs(f-1,2)+1) % gap before
                drawOneRegime(0,ts,S,regs(f-1,2)+1,indini-1,[],minimuminfo);
            end
            model = ModelFromCoeffs(regs(f,3:end));
            drawOneRegime(f,ts,S,indini,indfini,model,minimuminfo);
        end
        if regs(end,2) < N % trailing non-regime
            drawOneRegime(0,ts,S,regs(end,2)+1,N,[],minimuminfo);
        end
    end

end

function drawOneRegime(indreg,ts,S,indini,indfini,model,minimuminfo)
    ...
    if ~isempty(model) || modelbuilthere
        ...
        plot(xdrawings,yplotpdf,co,'LineWidth',4);
